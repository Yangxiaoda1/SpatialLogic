...
def load_model_on_gpu(gpu_id: int, model_name: str):
    ...
    checkpoint_path = "/apdcephfs_gy2/share_302507476/xiaodayang/SpatialLogic/ckpt/qwenvl/full/cot_only/checkpoint-51000"
    ...
    try:
        ...
        while True:
            try:
                ...
                # 构建prompt（去掉<answer>标签，与训练时保持一致）
                prompt = (
                    f"你是一位空间感知专家。\n请仔细观察并分别描述两张图片(img1和img2)。 提示: 这两张图片发生在同一个任务中，描述了不同的任务完成状态。\n任务名称: {task_name}\n描述要点:\n1.对img1 和 img2 进行详细的描述。\n2.基于上述描述，分析哪一张图片(img1 或img2)中的状态更接近任务的最终完成阶段。\n3.直接给出最终答案。\n输出格式:\nspatial details: [分别详细描述两张图片的空间细节]\ncloser to completion: [img1/img2]"
                )
...
def main():
    ...
    parser.add_argument("--gt_csv", type=str, default="/apdcephfs_gy2/share_302507476/xiaodayang/SpatialLogic/eval/gt_demo.csv")
    parser.add_argument("--pred_csv", type=str, default="/apdcephfs_gy2/share_302507476/xiaodayang/SpatialLogic/eval/pred_8gpu_demo.csv")
