...
class VideoFinetuneDataset(Dataset):
    def __init__(self, processor, image_path, task_path, window_sizes=[5,6,7,8,9,10,11,13,14,15], max_length=128):
        ...
        for sub1 in os.listdir(image_path):  # 327
            ...
            for sub2 in os.listdir(sub1path):  # 648642
                ...
                for sub3 in os.listdir(sub2path): #Place the held corn into the shopping cart's plastic bag.
                    sub3path = os.path.join(sub2path, sub3)
                    baseid=int(min(os.listdir(sub3path)).split('.')[0])
                    # print(f"baseid:{baseid}")
                    num_frames = len(os.listdir(sub3path))
                    for window_size in self.window_sizes:
                        ...
                        for i in range(num_frames - window_size + 1):
                            ...
                            first_img = os.path.join(sub3path, f"{baseid+i:05d}.jpg")
                            last_img = os.path.join(sub3path, f"{baseid+i + window_size - 1:05d}.jpg")
                            sample1 = {
                                "messages": [{
                                    "role": "user",
                                    "content": [
...
if __name__ == "__main__":
    ...
    parser.add_argument("--window_size", type=list, default=[6,7,8,9,10,11,12,13,14], help="滑动窗口大小，默认5")
    parser.add_argument("--max_length", type=int, default=128, help="文本部分最大长度，默认128")
    parser.add_argument("--output_dir", type=str, default="/home/tione/notebook/SpacialLogic-Demo/qwenvl/lora/mycheckpoint", help="输出目录")
    parser.add_argument("--epochs", type=int, default=10, help="训练 epoch 数量")
    parser.add_argument("--batch_size", type=int, default=32, help="每个设备的 batch 大小")
